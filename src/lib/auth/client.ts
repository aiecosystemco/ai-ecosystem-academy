...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    ...
    onSuccess(ctx) {
      // Email/password (and some broker responses) return the session as
      // `set-auth-token`. The live preview iframe cannot keep `__Host-`
      // cookies, so persist the token the same way Google/X popup sign-in does.
...
/** Keep a Better Auth session token for the live-preview iframe (no cookies). */
export function rememberBearerToken(token: string | null | undefined): void {
  if (!token) return;
